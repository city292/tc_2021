...
def init_weights(net, init_type='normal', gain=0.02):
    ...
    # logging.info('Network initialized with weights sampled from N(0,0.02).')
    net.apply(init_func)


def init_network(net, npgus=1):
    net.cuda()
    if npgus > 0:
        net = torch.nn.DataParallel(net, list(range(npgus)))
    return net
...
def set_train(nets, ops='train'):
    if ops == 'train' or ops is True:
        for net in nets:
            net.train()
    elif ops == 'eval' or ops is False:
        for net in nets:
            net.eval()
